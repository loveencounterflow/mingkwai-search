...
(function() {
  ...
  this.write_http_head = function(response, status_code, cookie) {
    ...
    if (cookie != null) {
      headers['Cookie'] = (function() {
        ...
        for (name in cookie) {
          value = cookie[name];
          results.push(COOKIE.serialize(name, value));
        }
        return results;
      })();
    }
  ...
  this.respond = function(request, response, uid, url) {
    var all_ds_infos, buffer, cookie, do_search_db, ds_infos, dsids, finalize_response, gids, html, html_front, html_rear, last_idx, last_query, parsed_url, q, ref, ref1, ref2, result_count, route, rqid, search_ds_file, send_buffer, t0, tainted_languagecode, tasks, Δ;
    t0 = 1 * new Date();
    rqid = "RQ#" + server['info']['request-count'];
    parsed_url = njs_url.parse(url, true);
    route = parsed_url['pathname'];
    last_query = parsed_url['query'];
    q = (ref = last_query['q']) != null ? ref : null;
    ...
    tainted_languagecode = (ref1 = cookie['languagecode']) != null ? ref1 : 'en_US';
    (function(_this) {
      return (function(dsid, ds_info, ds) {
        var i, len, ref2, results;
        ds = (ref2 = last_query['ds']) != null ? ref2 : [];
        ...
        for (i = 0, len = ds.length; i < len; i++) {
          ...
          dsids[dsid] = 1;
          results.push(ds_infos.push(ds_info));
        }
        return results;
      });
    })(this)(null, null, null);
    ...
    (function(_this) {
      return (function(gid, ds_info, dg) {
        var i, len, ref2, results;
        dg = (ref2 = last_query['dg']) != null ? ref2 : [];
        if (!TYPES.isa_list(dg)) {
          dg = [dg];
        }
        results = [];
        for (i = 0, len = dg.length; i < len; i++) {
          gid = dg[i];
          if (DSREGISTRY['groupname-by-gid'][gid] == null) {
            continue;
          }
          results.push(gids[gid] = 1);
        }
        return results;
      });
    })(this)(null, null, null);
    ...
    ref2 = html.split(this._cut_here_matcher), html_front = ref2[0], html_rear = ref2[1];
    if (html_rear == null) {
